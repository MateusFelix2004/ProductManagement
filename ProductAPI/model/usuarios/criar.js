...
function criarUsuario(email, senha) {
  return new Promise((resolve, reject) => {
    ...
    let permissao = 2; // Define o nível de permissão do usuário. Valor fixo para leitura, criação e edição.

    // Executa a consulta SQL para criar um novo usuário no banco de dados
    dbconn.query('CALL adicionarUsuario( ?, ?, ?, ?)', [email, hashSenha, dataAtual, permissao], (error, results) => {
      if (error) {
