        ...
        if (usuario != null && usuario != false) {
            ...
            if (senhaHashada === hash) {
                const payload = {
                    ...
                    permissao: usuario.permissao, // permissão do usuário
                    registro: usuario.registro // ata de registro do usuario
                };

                const token = gerarToken(payload); // Obtém um token para o usuário com expiração em 1 hora

                // Devolve o token ao cliente
                return res.status(200).json({ token: token, id: usuario.id, permissao: usuario.permissao });
            } else {
            ...
            // Verifica se o 'usuario' é null ou vazio
            return res.status(401).json({ mensagemErro: 'Usuário ou senha incorretos.' });
        } else {
